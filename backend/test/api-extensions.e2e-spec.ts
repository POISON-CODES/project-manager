...
describe('API Extensions (e2e)', () => {
  ...
  afterAll(async () => {
    ...
    await prisma.comment.deleteMany({
      where: { taskId: { in: [testTask?.id].filter(Boolean) } },
    });
  ...
  it('/users (GET) - should return list of users', async () => {
    ...
    expect(response.body.data.some((u: any) => u.id === testUser.id)).toBe(
      true,
    );
  });
  ...
  it('/tasks (GET) - should support filtering', async () => {
    ...
    const statusRes = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(statusRes.body.data.some((t: any) => t.id === testTask.id)).toBe(
      true,
    );
    expect(
      statusRes.body.data.every(
